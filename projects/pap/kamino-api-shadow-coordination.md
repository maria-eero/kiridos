...
**Make-or-break validation — CONFIRMED (Mitch Lee, Aug 27, 2026): Tipoca supports PAP.** The `TipocaServiceModel` exposes an **`accountPool`** field on `AccountProperties` (model main.xml L237) and `DomainProperties` (L248, alongside `authorizationSource` + `capacity`), plus a V2 API surface (`GenerateAccountV2` / `BatchGenerateAccountV2` / `ProvisionTestAccount`). So accounts can be vended into a named pool. For eero stage: **`accountPool = eeroStage`, `marketplaceId = A10ZONQX51YR1E`**. Wiring: register a **PAP-bound domain** for your AAA identity (`DomainProperties` binds domain→accountPool→authorizationSource; not the `DOMAIN_FREE_CREATION` shortcut), or pass `accountPool` to the V2 generate ops. Mitch's `authx-tipoca-mcp-server` currently sets only `marketplaceId`+`domainId`, so it needs a small tweak to pass `accountPool`.

**Also confirmed (Kamino 2.0 wiki + Audible thread):** Kamino stops onboarding new CI/CD (pipeline) clients as of April 2026, and Kamino/KILO are on a deprecation path — so **Tipoca-direct is the correct, future-proof choice** (Kamino is just a decorator layer over Tipoca). MosaicD (DA platform, eero already a client) is for digital-purchase/subscription test data, not auth pools — not applicable to PAP login, but relevant for future eero Plus subscription test data. Mock-payment (Bertha→PVF) constraints only matter if login tests need a subscription.
